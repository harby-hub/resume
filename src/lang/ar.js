export default {
	isoName    : 'en'      ,
	nativeName : 'العربية' ,
	rtl        : true      ,
	MainLayout : {
		title        : 'محمود سامي'              ,
		contactMe    : 'اتصل بي'                 ,
		clickCopy    : 'انقر هنا للنسخ'          ,
		copped       : 'قطع'                     ,
		phoneNumber  : 'رقم الهاتف'              ,
		emailAddress : 'عنوان البريد الإلكتروني' ,
		connect      : 'الاتصال'                  ,
		linkedinLink : 'رابط ينكدين'             ,
		githubLink   : 'رابط جيثب'               ,
		youtubeLink  : 'رابط يوتيوب'             ,
	},
	Index : {
		experience               : 'خبرة'                                                                                                                                                                ,
		curren                   : 'اللي الان'                                                                                                                                                               ,
		analyzeProjects          : 'تحليل المشاريع وتصميم قواعد البيانات ، وبناء الواجهة الخلفية وواجهة برمجة تطبيقات مريحة وواجهة رسم بياني لتطبيقات الأجهزة المحمولة ، وإنشاء عرض لواجهة مستخدم الويب' ,
		WebDeveloperFreelancer   : 'مطور ويب مستقل وتعلم ذاتي'                                                                                                                                           ,
		PROJECTS                 : 'المشاريع'                                                                                                                                                            ,
		fullStackWebDeveloper    : 'مطور ويب كامل'                                                                                                                                                  ,
		backEndWebDeveloper      : 'مطور الويب الخلفي'                                                                                                                                                   ,
		frontEndWebDeveloper     : 'مطور الويب الأمامي'                                                                                                                                                  ,
		UsedTechnologies         : 'التقنيات المستخدمة'                                                                                                                                                  ,
		backEnd                  : 'النهاية الخلفية'                                                                                                                                                     ,
		frontEnd                 : 'نهاية المقدمة'                                                                                                                                                       ,
		packages                 : 'الحزم'                                                                                                                                                               ,
		freelance                : 'حسابهم الخاص'                                                                                                                                                        ,
		LinuxOperatingSystem     : 'أوصاف نظام تشغيل Linux'                                                                                                                                              ,
		DevOpsPackages           : 'حزم DevOps'                                                                                                                                                          ,
		nativeProgramingLanguage : 'لغة البرمجة الأم'                                                                                                                                                    ,
		frameworks               : 'إطار أعمال'                                                                                                                                                          ,
		databases                : 'قواعد بيانات'                                                                                                                                                        ,
		UnitTesting              : 'وحدة التجارب'                                                                                                                                                        ,
		skils                    : [
			'تجربة مع RESTful APIs و GraphQL APIs'                               ,
			'المصادقة بين أنظمة أساسية متعددة => OAuth 2.0'                      ,
			'تصميم قواعد البيانات والنمذجة والتطبيع'                             ,
			'اعمل مع قواعد بيانات الملايين من الصفوف واكتب استعلامات SQL محسّنة'    ,
			'جيد في هياكل البيانات والخوارزميات'                                 ,
			'كود نظيف وموثق'                                                     ,
			'أنماط التصميم ومبادئ SOLID'                                         ,
			'معرفة جيدة في vuejs'                                                ,
		],
	}
}